...
class DoubleGoodsTableViewCell: UITableViewCell {
    ...
    class func configCell(cell:DoubleGoodsTableViewCell!,product : ZMDProduct!,productR : ZMDProduct?) {
        let size = product.Name.sizeWithFont(UIFont.systemFontOfSize(15), maxWidth: 200)
...
//            product.Name
        ...
        if let pictureModel = product.DefaultPictureModel {
            ...
            cell.goodsImgVLeft.setImageWithURL(NSURL(string: imgUrl), placeholderImage: UIImage(named: "product_default"))
        }
        ...
        if let pictureModel = productR.DefaultPictureModel {
            let imgUrl = kImageAddressMain + (pictureModel.ImageUrl ?? "")
            cell.goodsImgVRight.setImageWithURL(NSURL(string: imgUrl), placeholderImage: UIImage(named: "product_default"))
        }
