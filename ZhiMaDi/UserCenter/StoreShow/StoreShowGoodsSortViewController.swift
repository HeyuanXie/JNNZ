...
//        return self.dataArray2[section].count
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if indexPath.section == 0{
            ...
            cell?.textLabel?.text = self.dataArray1[indexPath.section] as? String
            return cell!
        } else {
            if indexPath.row == 0 {
                let cellId = "headCell"
                ...
                if cell == nil {
                    ...
                    let size = btnTitle.sizeWithFont(defaultTextSize, maxWidth: 140)
                    let leftBtn = UIButton(frame: CGRect(x: MarginLeft, y: 10, width: Int(size.width)+20, height: 30))
                    leftBtn.backgroundColor = RGB(62,182,35,1.0)
...
//                cell?.selectionStyle = .None
                ...
                let getBtn = { (text : String,index : Int) -> UIButton in
                    ...
                    btn.rac_signalForControlEvents(.TouchUpInside).subscribeNext({ (sender) -> Void in
                        ...
                        let title = self.dataArray2[indexPath.section-1][index] as! String//index = sender.tag
                        let homeBuyListViewController = HomeBuyListViewController.CreateFromMainStoryboard() as! HomeBuyListViewController
                        homeBuyListViewController.isStore = true
                        homeBuyListViewController.titleForFilter = title
                        homeBuyListViewController.Cid = self.dataArray3[indexPath.section] as! String
...
//            cell?.contentView.addSubview(line)
    ...
    private func subViewInit(){
        self.title = "商品分类"
        self.currentTableView = UITableView(frame: CGRect(x: 0, y: 0, width: kScreenWidth, height: kScreenHeight), style: .Plain)
