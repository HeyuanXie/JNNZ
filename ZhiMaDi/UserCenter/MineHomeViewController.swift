...
//        }
        ...
        self.btn.userInteractionEnabled = g_isLogin! ? false : true
...
//            leftItem.customView?.tintColor = defaultDetailTextColor
...
//        self.userCenterData = [[.UserHead,.UserMyOrder,.UserMyOrderMenu],[.UserWallet,.UserCardVolume,.UserMyCrowdFunding],[.UserMyStore,.UserVipClub,.UserCommission,.UserInvitation],[.UserHelp],[.UserMore]]
        ...
        if g_isLogin! {
            //获取订单数目
            QNNetworkTool.getOrderNumbers(1, orderNo: "", pageIndex: 0, pageSize: 1) { (dic, Error) -> Void in
                if let dict = dic, orderNumberDict = dict["CustomerOrderStatusModel"] {
                    self.orderNumberDic = NSMutableDictionary(dictionary: orderNumberDict as! [NSObject : AnyObject])
                    self.currentTableView.reloadData()
                }
            }
        }
