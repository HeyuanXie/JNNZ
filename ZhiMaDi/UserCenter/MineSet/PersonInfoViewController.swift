...
//    var postHeadImage : ((imageData: NSData) -> Void)! //设置图像后将iamgeData传递给MineSetVC
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        case .Head://选择图像
            ...
            if self.headerView.image == nil {
                if let urlStr = g_customer?.Avatar?.AvatarUrl,url = NSURL(string: urlStr) {
                    headerView.sd_setImageWithURL(url, placeholderImage: nil)
                }
            }
        case .NickN:
            cell.accessoryType = UITableViewCellAccessoryType.DisclosureIndicator
            ...
            self.nameLB.text = g_customer?.FirstName ?? ""  //FirstName为电话号码
            if g_isLogin! {
        ...
        case .Version:
            ...
            let versionLbl = ZMDTool.getLabel(CGRectMake(kScreenWidth - 100 - 58, 0, 120, tableViewCellDefaultHeight), text: "V "+APP_VERSION_BUILD, fontSize: 17, textColor: defaultDetailTextColor, textAlignment: .Right)
