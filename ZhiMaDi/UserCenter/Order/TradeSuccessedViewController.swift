...
class TradeSuccessedViewController: UIViewController, UITableViewDataSource, UITableViewDelegate,ZMDInterceptorMoreProtocol {
    ...
    func updateUI(){
        self.title = "交易成功"
        self.currentTableView = UITableView(frame: CGRect(x: 0, y: 0, width: kScreenWidth, height: self.view.bounds.height), style: UITableViewStyle.Plain)
...
//        self.updateFoot()
    ...
    func updateFoot() {
        ...
        let titles = ["立即评价","继续购物"]
        for i in 0..<2 {
            let btn = UIButton(frame: CGRect(x: CGFloat(i)*width, y: 0, width: width, height: view.frame.height))
            btn.tag = 1000 + i
