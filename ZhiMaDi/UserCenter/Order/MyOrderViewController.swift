...
class MyOrderViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,ZMDInterceptorProtocol {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.updateUI()
        self.currentOrderStatu = self.orderStatuId == 4 ? .UnComment : OrderStatu(value: self.orderStatusIndex)
    }
    ...
    func numberOfSectionsInTableView(tableView: UITableView) -> Int {
        return self.isAfterSale ? 0 : self.dataArray.count  //TODO:暂时售后是没有的
    }
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if indexPath.row <  Items.count+1 && indexPath.row != 0 {
            ...
            vc.productId = Items[indexPath.row-1].ProductId.integerValue
    ...
    func cellForTotal(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if self.isAfterSale {
            ...
            if cell == nil {
                ...
                var count = 0
                for text in texts {
                    let label = ZMDTool.getLabel(CGRect(x: 12+width*CGFloat(count), y: 0, width: width, height: 56), text: text, fontSize: 14, textColor: defaultTextColor, textAlignment: .Right)
            ...
            let refundLbl = cell?.contentView.viewWithTag(10001) as! UILabel
            let dealLbl = cell?.contentView.viewWithTag(10000) as! UILabel
            refundLbl.text = texts[1]
            dealLbl.text = texts[0]
            return cell!
        }
        let cellId = "TotalCell"
        var cell = tableView.dequeueReusableCellWithIdentifier(cellId)
    ...
    func cellForStatu(tableView: UITableView, cellForRowAtIndexPath indexPath:NSIndexPath) ->UITableViewCell {
        ...
        if cell == nil {
            ...
            statuLbl.tag = 10002
            let statuImg = UIImageView(image: UIImage(named: "order_status"))
            ...
            orderNumberLbl.snp_makeConstraints(closure: { (make) -> Void in
                ...
                make.top.equalTo(0)
                make.right.equalTo(statuImg).offset(3)
        ...
        for title in titles {
            ...
            btn.rac_command = RACCommand(signalBlock: { (sender) -> RACSignal! in
                if titles[sender.tag-1000] == "订单详情" {
                    ...
                    let alert = UIAlertController(title: "提醒", message: "确定取消此订单?", preferredStyle: .Alert)
                    let action1 = UIAlertAction(title: "确定", style: .Destructive, handler: { (sender) -> Void in
                        ZMDTool.showActivityView(nil)
