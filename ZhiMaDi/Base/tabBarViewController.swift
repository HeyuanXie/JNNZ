...
private enum QNTabBarItem: Int {
    ...
    var imageName: String {
        switch self {
        case .Home: return "Tab-bar01"
        case .Category: return "Tab-bar02"
        case .School: return "Tab-bar03"
        case .Mine: return "Tab-bar04"
        }
    }
}
...
//        }
    ...
    private func itemConfig(qnItem: QNTabBarItem, haveDot: Bool = false) -> UITabBarItem? {
        if let item = self.tabBar.items?[qnItem.rawValue]{
            ...
            if let selectedImage = UIImage(named: imageName),let image = UIImage(named: imageName+imageName.componentsSeparatedByString("0").last!) {
                    if !haveDot {
